...
from nonebot.adapters.onebot.v11 import Bot, GroupIncreaseNoticeEvent, \
    MessageSegment, Message, GroupMessageEvent, Event, escape, PrivateMessageEvent
...
# 功能块，比如说下面这个最简单的功能块就是用户发七七机器人回复我在
# 第一行是on，表条件，除了第一行后面就是满足条件执行的内容
w = on_fullmatch('七七')   # 完全匹配，用户发送的消息=七七
...
w = on_keyword({'今日人品', '今日运势'}, priority=5)   # 关键词匹配，用户发送的消息含有今日人品或今日运势
# 只有关键词匹配支持多个关键词
...
w = on_keyword({'原神'}, priority=5)   # 关键词匹配，用户发送的消息含有原神
lt = 0
...
pp = on_command('匹配', priority=5)  #命令匹配，/开头的消息
# /匹配
# /匹配 xxx
...
@abstract.handle()
async def fudu(bot: Bot, event: MessageEvent):
    await w.send("此功能暂时不可用。")
# @abstract.handle()
# async def _(state: T_State, arg: Message = CommandArg()):
#     if arg.extract_plain_text().strip():
#         state["abstract"] = arg.extract_plain_text().strip()
...
w = on_keyword({'复读机'}, priority=5)
@w.handle()
async def fudu(bot: Bot, event: MessageEvent):
    await w.send(str(w))  # 直接将 w 转为字符串并发送
    await w.send("请说")
